# main.py - منظومة Ahemmad المتكاملة والمحدثة 2025
import os
import re
import time
import random
...
# --- أنظمة الحماية (Protection Modules) ---
async def check_for_links(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if URL_REGEX.search(text) or (message.entities and any(e.type in ['url', 'text_link'] for e in message.entities)):
        try:
            await message.delete()
            await context.bot.send_message(update.effective_chat.id, f"🚫 ممنوع نشر الروابط!")
            return True
        except: return True
    return False
...
async def handle_greetings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # الردود (تعمل للجميع بما فيهم المطور)
...
async def xo_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تصحيح: انتظار انضمام اللاعب O
    if game['player_o'] is None:
        if uid != game['player_x']:
            game['player_o'] = uid
            await query.answer("انضممت كلاعب O! دور X الآن.")
        else:
            await query.answer("🚫 بانتظار انضمام لاعب آخر!", show_alert=True); return

    if (game['turn'] == 'X' and uid != game['player_x']) or (game['turn'] == 'O' and uid != game['player_o'] and game['player_o'] != BOT_O_ID):
        await query.answer("🚫 ليس دورك!", show_alert=True); return
...
def main():
    ...
    # الأوامر الأساسية
    app.add_handler(CommandHandler("start", lambda u,c: u.message.reply_text("🛡️ بوت Ahemmad يعمل بنجاح!")))
    ...
    # ترتيب المعالجات (الأهمية القصوى)
    app.add_handler(MessageHandler(filters.TEXT & filters.ChatType.GROUPS & ~filters.COMMAND, protection_handler), group=1)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_greetings), group=2)

    app.run_polling()

# (دوال تشغيل XO المساعدة كما في كودك الأصلي)
